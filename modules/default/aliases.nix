  ...
  config = lib.mkIf config.glf.aliases.enable {
    ...
    environment = {
      ...
      shellAliases = {
        ...
        glf-fixkde = "sed -i 's/file:\/\/\/nix\/store\/[^\/]*\/share\/applications\//applications:/gi'       /home/$USER/.config/plasma-org.kde.plasma.desktop-appletsrc       && systemctl restart --user plasma-plasmashell";
