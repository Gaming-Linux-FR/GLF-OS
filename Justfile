...
install:
	...
	if [ -n "$ISO_FILE" ]; then
		...
		cp "$ISO_FILE" "GLF-OS-BETA-OMNISLASH_STABLE.iso" # Copie avec la nomination GLF OS
		sha256sum "GLF-OS-BETA-OMNISLASH_STABLE.iso" > "GLF-OS-BETA-OMNISLASH_STABLE.iso.sha256sum"
		cat "GLF-OS-BETA-OMNISLASH_STABLE.iso.sha256sum"
	else
